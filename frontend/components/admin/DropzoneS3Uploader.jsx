...
export default class DropzoneS3Uploader extends React.Component {
  ...
  handleFinish = (info, file) => {
    const regex = /(?:(?!\?).)*/;
    const filename = info.signedUrl.match(regex)[0];
  ...
  renderImage = ({uploadedFile}) => (<div className="rdsu-image"><img height="200px" wdith="200px" src={uploadedFile.fileUrl} /></div>)
  ...
  render() {
    ...
    return (
         <Dropzone getDataTransferItems={evt =>
                 getDroppedOrSelectedFiles(evt).then(list =>
                   list.map(({ fileObject, ...rest }) => fileObject)
                 )
               } ref={c => this._dropzone = c} onDrop={this.handleDrop} {...dropzoneProps}>
           {content}<p className="dropzone-text">Drop files here</p>
         </Dropzone>
    )
  }
}
